 ...
 * 贾运通 2018/12/31
 */
app.controller('itemCatAuditController', function ($scope, $controller, itemCatAuditService, typeTemplateService) {

    $controller('baseController', {$scope: $scope});//继承

    $scope.searchEntity = {parentId: 0};
    $scope.search = function (currentPage, itemsPerPage) {
        itemCatAuditService.searchItemCatAudit(currentPage, itemsPerPage, $scope.searchEntity).success(function (response) {
    ...
    $scope.selectItemCatsMapByParentId = function () {
        itemCatAuditService.selectItemCatsMapByParentId(0).success(function (response) {
    ...
    $scope.$watch("grandParentId", function (newValue, oldValue) {
        if (!newValue > 0) {
            ...
            itemCatAuditService.selectItemCatsMapByParentId(newValue).success(function (response) {
    ...
    $scope.saveItemCat = function () {
        ...
        if ($scope.entity.id != null) {
            method = itemCatAuditService.updateItemCat($scope.entity); // 修改
        } else {
            method = itemCatAuditService.insertItemCat($scope.entity); // 增加
        }
    ...
    $scope.selectItemCatById = function (id) {
        itemCatAuditService.selectItemCatById(id).success(
            function (response) {
                // 根据传入的id回显数据
                $scope.entity = response;
                // 如果传入的id父id不为0,则说明不是一级目录
                if ($scope.entity.parentId != 0) {
                    itemCatAuditService.selectItemCatById($scope.entity.parentId).success(function (response) {
    ...
    $scope.dele = function () {
        //获取选中的复选框
        itemCatAuditService.dele($scope.selectIds).success(
            function (response) {
                if (response.flag) {
