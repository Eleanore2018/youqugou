app.controller("goodsController", function ($scope, $controller, $location,itemService, typeTemplateService, itemCatService, uploadService, goodsService) {
    ...
    //查询实体
    $scope.selectGoodsVoById = function () {
        var id = $location.search()['id'];
        if (null == id) {
            return;
        }
        goodsService.selectGoodsVoById(id).success(
            function (response) {
    ...
    $scope.save = function () {
        ...
        if ($scope.entity.goods.id != null) {//如果有ID
            serviceObject = goodsService.updateGoodsVo($scope.entity); //修改
        } else {
            serviceObject = goodsService.insertGoodsVo($scope.entity);//增加
        }
    ...
    //批量删除
    $scope.deleteGoodsVo = function () {
        //获取选中的复选框
        goodsService.deleteGoodsVo($scope.selectIds).success(
            function (response) {
                if (response.flag) {
    ...
    $scope.search = function (currentPage, itemsPerPage) {
        goodsService.searchGoodsPage(currentPage, itemsPerPage, $scope.searchEntity).success(
            function (response) {
                $scope.list = response.rows;
                $scope.paginationConf.totalItems = response.total;//更新总记录数
            }
    ...
    $scope.selectItemCat1List = function () {
        itemCatService.selectItemCatsByParentId(0).success(function (response) {
    ...
    $scope.$watch("entity.goods.category1Id", function (newValue, oldValue) {
        itemCatService.selectItemCatsByParentId(newValue).success(function (response) {
    ...
    $scope.$watch("entity.goods.category3Id", function (newValue, oldValue) {
        itemCatService.selectItemCatById(newValue).success(function (response) {
    ...
    $scope.$watch("entity.goods.typeTemplateId", function (newValue, oldValue) {
        // 根据模板ID查询模板的数据
        typeTemplateService.selectTypeTemplateById(newValue).success(function (response) {
        ...
        // 根据模板ID获得规格的列表的数据：
        typeTemplateService.selectSpecListById(newValue).success(function (response) {
  ...
  /*  // 用于回显品牌集合
    ...
    // 显示状态
    // $scope.selectIds = [];
    $scope.status = ["未审核", "审核通过", "审核未通过", "关闭", "上架", "下架"];

    $scope.putOnSale = function (status) {
        goodsService.putOnSale($scope.selectIds,status).success(function (response) {
            if (response.flag) {
                $scope.reloadList();
                $scope.selectIds = [];
            } else {
    ...
    $scope.undercarriage = function (status) {
        goodsService.undercarriage($scope.selectIds,status).success(function (response) {
            if (response.flag) {
    ...
    $scope.itemCatList = [];
    // 显示分类:
    $scope.selectAllItemCats = function () {
        itemCatService.selectAllItemCats().success(function (response) {
